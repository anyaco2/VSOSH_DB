...
public class PassingPointsService : IPassingPointsService
{
	...
	private async Task<IReadOnlyCollection<IGrouping<int, SchoolOlympiadResultBase>>> GetResults(Subject subject, CancellationToken cancellationToken)
	{
		...
		{
			Subject.Art => await _resultRepository.FindWithGroupByGradeCompeting<ArtResult>(cancellationToken: cancellationToken),
			Subject.Astronomy => await _resultRepository.FindWithGroupByGradeCompeting<AstronomyResult>(cancellationToken: cancellationToken),
			Subject.Biology => await _resultRepository.FindWithGroupByGradeCompeting<BiologyResult>(cancellationToken: cancellationToken),
			Subject.Chemistry => await _resultRepository.FindWithGroupByGradeCompeting<ChemistryResult>(cancellationToken: cancellationToken),
			Subject.Chinese => await _resultRepository.FindWithGroupByGradeCompeting<ChineseResult>(cancellationToken: cancellationToken),
			Subject.ComputerScience => await _resultRepository.FindWithGroupByGradeCompeting<ComputerScienceResult>(cancellationToken: cancellationToken),
			Subject.Ecology => await _resultRepository.FindWithGroupByGradeCompeting<EcologyResult>(cancellationToken: cancellationToken),
			Subject.Economy => await _resultRepository.FindWithGroupByGradeCompeting<EconomyResult>(cancellationToken: cancellationToken),
			Subject.English => await _resultRepository.FindWithGroupByGradeCompeting<EnglishResult>(cancellationToken: cancellationToken),
			Subject.French => await _resultRepository.FindWithGroupByGradeCompeting<FrenchResult>(cancellationToken: cancellationToken),
			Subject.FundamentalsLifeSafety => await _resultRepository.FindWithGroupByGradeCompeting<FundamentalsLifeSafetyResult>(cancellationToken: cancellationToken),
			Subject.Geography => await _resultRepository.FindWithGroupByGradeCompeting<GeographyResult>(cancellationToken: cancellationToken),
			Subject.German => await _resultRepository.FindWithGroupByGradeCompeting<GermanResult>(cancellationToken: cancellationToken),
			Subject.History => await _resultRepository.FindWithGroupByGradeCompeting<HistoryResult>(cancellationToken: cancellationToken),
			Subject.Law => await _resultRepository.FindWithGroupByGradeCompeting<LawResult>(cancellationToken: cancellationToken),
			Subject.Literature => await _resultRepository.FindWithGroupByGradeCompeting<LiteratureResult>(cancellationToken: cancellationToken),
			Subject.Math => await _resultRepository.FindWithGroupByGradeCompeting<MathResult>(cancellationToken: cancellationToken),
			Subject.PhysicalEducation => await _resultRepository.FindWithGroupByGradeCompeting<PhysicalEducationResult>(cancellationToken: cancellationToken),
			Subject.Physic => await _resultRepository.FindWithGroupByGradeCompeting<PhysicResult>(cancellationToken: cancellationToken),
			Subject.Russian => await _resultRepository.FindWithGroupByGradeCompeting<RussianResult>(cancellationToken: cancellationToken),
			Subject.SocialStudies => await _resultRepository.FindWithGroupByGradeCompeting<SocialStudiesResult>(cancellationToken: cancellationToken),
			Subject.Technology => await _resultRepository.FindWithGroupByGradeCompeting<TechnologyResult>(cancellationToken: cancellationToken),
